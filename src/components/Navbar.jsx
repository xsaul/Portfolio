...
const Navbar = ({ darkMode, setDarkMode }) => {
  ...
  <div className="flex justify-between items-center">
    ...
    <ul className={`${menuOpen ? "flex" : "hidden"} md:flex flex-col md:flex-row items-stretch md:items-center gap-2 w-full md:w-auto absolute md:relative top-full left-0 md:top-auto md:left-auto p-6 md:p-0 bg-canvas dark:bg-midnight md:bg-transparent dark:md:bg-transparent border-b md:border-b-0 border-black/[0.08] dark:border-white/[0.1] shadow-lg md:shadow-none`}>
      <li>
        <button
          onClick={() => setDarkMode(!darkMode)}
          className="w-full md:w-auto p-2 rounded-btn bg-white dark:bg-midnight border border-black/[0.08] dark:border-white/[0.1] text-black dark:text-white transition-colors duration-200 flex justify-center"
